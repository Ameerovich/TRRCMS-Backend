...
| `schema_version` | `1.2.0` | Schema version of the .uhc format |
...
| `head_of_household_name` | TEXT | Yes | Name of household head |
| `household_size` | INTEGER | Yes | Total household members |
| `head_of_household_person_id` | TEXT (UUID) | No | FK → `persons.id` |
...
| `id` | TEXT (UUID) | Yes | Primary key |
| `property_unit_id` | TEXT (UUID) | Yes | FK → `property_units.id` |
| `claim_type` | TEXT | Yes | Claim type string: `'Ownership'` or `'Occupancy'` |
...
> **Hardcoded enums (not vocabulary-driven):** `gender`, `nationality`, `relationship_to_head` on `persons` use fixed integer codes defined in the schema tables above — they are NOT downloaded as vocabularies. `claim_type` on `claims` is a TEXT string (`'Ownership'` or `'Occupancy'`), also not vocabulary-driven.
...
persons
  ...
  ├── households              (head_of_household_person_id → persons.id)
  │                           (household_id → households.id — on persons table)
...
CREATE TABLE households (
    ...
    head_of_household_person_id  TEXT REFERENCES persons(id),
...
- [ ] Hardcoded enum fields (`gender`, `nationality`, `relationship_to_head`) use correct integer codes
- [ ] `claim_type` uses correct TEXT values (`'Ownership'` or `'Occupancy'`)
...
- [ ] Manifest table populated with all required keys (including `building_document_count`)
