        ...
        SqliteConnection connection, Guid packageId, CancellationToken ct)
    {
        ...
        while (await reader.ReadAsync(ct))
        {
            var entity = StagingPropertyUnit.Create(
                ...
                unitType: GetEnum<PropertyUnitType>(reader, "unit_type", "property_unit_type", PropertyUnitType.Unknown),
                status: GetEnum<PropertyUnitStatus>(reader, "status", "property_unit_status", PropertyUnitStatus.Unknown),
        ...
        SqliteConnection connection, Guid packageId, CancellationToken ct)
    {
        ...
        while (await reader.ReadAsync(ct))
        {
            var entity = StagingHousehold.Create(
                ...
                maleCount: GetNullableInt(reader, "male_count"),
                femaleCount: GetNullableInt(reader, "female_count"),
                adultCount: GetNullableInt(reader, "adult_count"),
                childCount: GetNullableInt(reader, "child_count"),
                elderlyCount: GetNullableInt(reader, "elderly_count"),
                disabledCount: GetNullableInt(reader, "disabled_count"),
                occupancyNature: GetNullableEnum<OccupancyNature>(reader, "occupancy_nature", "occupancy_nature"),
                occupancyStartDate: GetNullableDateTime(reader, "occupancy_start_date"),
                notes: GetNullableString(reader, "notes"));
        ...
        SqliteConnection connection, Guid packageId, CancellationToken ct)
    {
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            var entity = StagingIdentificationDocument.Create(
                ...
                originalEntityId: originalId,
                originalPersonId: GetGuid(reader, "person_id"),
                documentType: GetNullableEnum<DocumentType>(reader, "document_type", "document_type"),
                description: GetNullableString(reader, "description"),
                originalFileName: originalFileName,
                filePath: filePath,
                fileSizeBytes: fileSizeBytes,
