...
/// <summary>
/// Validator for CreateHouseholdInSurveyCommand (canonical v1.9 shape).
/// Upper-bound only rules — gaps allowed for unknown members.
/// </summary>
public class CreateHouseholdInSurveyCommandValidator : LocalizedValidator<CreateHouseholdInSurveyCommand>
{
    public CreateHouseholdInSurveyCommandValidator(IStringLocalizer<ValidationMessages> localizer, IVocabularyValidationService vocabService) : base(localizer)
    {
        ...
        RuleFor(x => x)
            .Must(x => x.MaleCount!.Value + x.FemaleCount!.Value == x.HouseholdSize)
        ...
        RuleFor(x => x)
            ...
            .When(x => (x.MaleCount.HasValue || x.FemaleCount.HasValue) && !(x.MaleCount.HasValue && x.FemaleCount.HasValue));
        ...
        RuleFor(x => x)
            .Must(x => x.AdultCount!.Value + x.ChildCount!.Value + x.ElderlyCount!.Value == x.HouseholdSize)
